...
#This script file is using Bag of Words and statistics for features
...
def convertCellTo1D(df, pos, c1, c2):
	...
	for row in df.itertuples():
		...
		j = row[pos]
		cellsArray = numpy.zeros(((c1 + 1)*(c2 + 1) + 4), int)
		...
		narray = cellsArray.flatten()
		m = numpy.mean(narray)
		ta = numpy.std(narray)
		lox = scipy.stats.skew(narray)
		kir = scipy.stats.kurtosis(narray)
		#print m, ta, lox, kir
		cellsArray[-4:] = float(m)
		cellsArray[-3:] = float(ta)
		cellsArray[-2:] = float(lox)
		cellsArray[-1:] = float(kir)
